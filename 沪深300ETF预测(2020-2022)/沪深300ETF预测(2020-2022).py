...
# 从tushare获取所需数据
def getData(list_code, stockcode='510300.SH',startdate='20220228',enddate='20220228'):
    ...
    print('大盘指数的每日行情数据长度',len(shdata['low']))

    # 拼接疫情数据
    shdata = get_Epidemic_data(shdata)

    print('拼接后',len(shdata['low']))
    ...
    indexdata={
        ...
        # 疫情数据
        'Asymptomatic':np.array(shdata['Asymptomatic'])[::-1],
        'Asymptomatic_new_cases':np.array(shdata['Asymptomatic_new_cases'])[::-1],
        'Cumulative_diagnosis':np.array(shdata['Cumulative_diagnosis'])[::-1],
        'New_diagnosis':np.array(shdata['New_diagnosis'])[::-1],
        'Cumulative_cure':np.array(shdata['Cumulative_cure'])[::-1],
        'New_cure':np.array(shdata['New_cure'])[::-1],
        'Cumulative_death':np.array(shdata['Cumulative_death'])[::-1],
        'New_death':np.array(shdata['New_death'])[::-1],
        'Local_diagnosis':np.array(shdata['Local_diagnosis'])[::-1],
        'New_local_diagnosis':np.array(shdata['New_local_diagnosis'])[::-1]
    }
    ...
    for tup in etf300:
        ...
        # print('个股资金流向长度：',len(mf['buy_sm_vol']))
        # print('指数应有数据长度：',len(flow['buy_sm_vol']))
...
def get_data_new():
    ...
    time_list = [
                 # ['20160101','20160105'],['20160701','20160703'],
    ...
    for i in time_list:
        ...
        data,indexdata,flow = getData(list, stockcode='510300.SH',startdate=start,enddate=end)
...
# 合并疫情数据
def get_Epidemic_data(quotation_data):

    # print(quotation_data['trade_date'])

    # print('行情数据：',quotation_data)
    epidemic_data = pd.read_csv('./全国.csv',encoding='gbk')
    epidemic_data['日期'] = epidemic_data['日期'].apply(lambda x: time.strftime('%Y%m%d',time.strptime(x,'%Y/%m/%d')))
    # print('疫情数据：',epidemic_data)
    epidemic_data = epidemic_data.rename(columns={'日期': 'trade_date'})
    epidemic_data = epidemic_data.fillna(0)
    quotation_data_new = pd.merge(left=quotation_data,right=epidemic_data,how='inner',on='trade_date')

    # 重新设置索引
    quotation_data_new = quotation_data_new.reset_index(drop=True)

    # print(quotation_data)
    # print('*****************')
    # print(quotation_data_new['trade_date'])
    # print('合并疫情数据后数据：', quotation_data_new)
    return quotation_data_new
